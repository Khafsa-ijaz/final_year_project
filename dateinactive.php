...
function Header()
{
   ...
   // $this->Ln(10);
    if ($this->PageNo()==1) {
     ...
     $date=$_POST['date'];
      ...
      $a111=date("M",strtotime($date1));
      $a211=date("Y",strtotime($date1));
      $a311=date("d",strtotime($date1));
      $format11=$a311."-".$a111."-".$a211;
   ...
   $this->SetTextColor(0,0,0);
    ...
    $this->Cell(160,10,'IN-ACTIVE EMPLOYEES LIST',0,0,'C');
     ...
     $this->Ln(7);
  $this->Cell(280,10,'DATE: ' .$format1.' *** '.$format11,0,0,'C');
    $this->Ln(7);
...
$sql = "SELECT * FROM employees where status='In-Active' and iad between '$date' and '$date1'  order by iad DESC";

...
if ($rowadv['pagename']=='Search Student')
{
   ...
   echo "<li><a class='menu-item' href='$rowadv[link]' data-i18n='nav.vertical_nav.vertical_nav_fixed'>$rowadv[pagename]</a>
        ...
        <div class="col-md-12">
            <div class="card">
              ...
              <div style="background-color: #5B8343" class="card-header">
                  <h4 style="color:white" class="card-title" id="horz-layout-colored-controls"><b><?php echo $name?></b> Files List</h4>
    ...
    <div class="row">
      ...
      <hr style="  border-top: 2px dotted #5B8343;">
            ...
            $sql = "SELECT * FROM extrastudents WHERE sname LIKE '%$name%'";
...
<td>
    ...
    <a target='_blank' data-toggle='tooltip' title=' Print All Files ' href=printallpdf.php?id=".$row['id']."> <button type='button' class='btn btn-icon btn-info btn-sm'><i class='fa fa-print'></i></button></a>

    <a target='_blank' data-toggle='tooltip' title='All Files List' href=viewfile2.php?id=".$row['id']."> <button type='button' class='btn btn-icon btn-success btn-sm'><i class='fa fa-file'></i></button></a>


         <a target='_blank' data-toggle='tooltip' title='View Uploaded Files List' href=viewfile.php?id=".$row['id']."> <button type='button' class='btn btn-icon btn-info btn-sm'><i class='fa fa-eye'></i></button></a>

         <a target='_blank' data-toggle='tooltip' title='Add Files' href=uploadfiles.php?id=".$row['id']."> <button type='button' class='btn btn-icon btn-warning btn-sm'><i class='fa fa-pencil'></i></button></a>


                    
                        <a target='_blank' data-toggle='tooltip' title='PDF REPORT' href=filespdf.php?id=".$row['id']."> <button type='button' class='btn btn-icon btn-dark btn-sm'><i class='fa fa-file'></i></button></a>

                             <button data-toggle='tooltip' title='Delete All files' type='button' value=Delete id=".$row['id']." class='btn btn-icon btn-danger btn-sm cc'><i class='fa fa-trash'></i></button>
...
$(".cc").click(function(e) {
 ...
 swal({
  title: "Are You Sure to Delete Students files?",
  icon: "warning",
  buttons: true,
  dangerMode: true,
})
 .then((willDelete) => {
  if (willDelete) {
     $.ajax({
                url:"delfilesandstudents.php?id="+del_id,
                cache:false,

		...
		<div class="col-md-12">
	        <div class="card">
              ...
              <div style="background-color: #f9d140" class="card-header">
                                   ...
                                   <h4 style="color:black" class="card-title" id="horz-layout-colored-controls"><b><u>Medicines List</b></u> <i><u></i></u></b></h4>
            ...
            <div   class="card">
                ...
                <div class="card-content collapse show">
                 ...
                 <div class="s text-center " role="" aria-label="Basic example"> 
                      <form method="POST" action="pdfledgerbankpetty">
                 
 <input type="hidden" value="<?php echo $bankname?>" name="bankname">
      ...
      <hr style="  border-top: 2px dotted #304A7B;">
                        ...
                        <table sty class="table table-striped table-bordered file-export">
                            <thead>
                                <tr>
                                   <th style="background-color:#304A7B;color: white ">BARCODE</th>
                                      <th style="background-color:#304A7B;color: white ">NAME</th>
                                    <th style="background-color:#304A7B;color: white  ">QTY</th>
                                     <th style="background-color:#304A7B;color: white  ">UNIT </th>
                                    <th style="background-color:#304A7B;color: white  ">CATEGORY</th>
                                          <th style="background-color:#304A7B;color: white  ">TYPE</th>
                                           <th style="background-color:#304A7B;color: white  ">LOCATION</th>
                                         

  <th style="background-color:#304A7B;color: white  ">DISPOSE</th>
  ...
  $count=0;
         ...
         $sql = "SELECT * FROM medicine3 order by name,cat";
            ...
            $sql2 = "SELECT * FROM medicine4 where bcode='$row[bcode]'";
...
if ($result2->num_rows > 0) {
  ...
  while($row2 = $result2->fetch_assoc()) {
         ...
         if ($row2['amountin']!='0')
{
    $balamt=$row2['amountin']+$balamt;
}

if ($row2['amountout']!='0')
{
    $balamt=$balamt-$row2['amountout'];
}
           ...
           echo " <tr>
                  ...
                  <div class='btn-group' role='group' aria-label='Basic example'>
                    <a data-toggle='tooltip' title='Add Dispose' href=dispose1.php?id=".$row['bcode']."><button type='button' class='btn btn-icon btn-primary btn-sm'><i class='   fa fa-recycle'></i></button></a>
                    
                    &nbsp
                     <a data-toggle='tooltip' title='Delete Dispose' href=itablefd.php?id=".$row['bcode']."><button type='button' class='btn btn-icon btn-info btn-sm'><i class='   fa fa-undo'></i></button></a>
                     </div>
...
<div class='btn-group' role='group' aria-label='Basic example'>
            <a data-toggle=tooltip' title='EDIT' href=editmed.php?id=".$row['id']."><button type='button' class='btn btn-icon btn-outline-warning btn-sm'><i class='fa fa-pencil'></i></button></a>

                  <button data-toggle='tooltip' title='Delete' type='button' value=Delete id=".$row['bcode']." class='btn btn-icon btn-outline-danger btn-sm cc'><i class='fa fa-trash'></i></button></td>
                  </div>
...
$(document).on('click','.cc',function(event){
 ...
 swal({
  title: "Are You Sure to Delete Medicine?It will delete all records with patiens all etc so be carefull",
  icon: "warning",
  buttons: true,
  dangerMode: true,
})
 .then((willDelete) => {
  if (willDelete) {
     $.ajax({
                url:"deleteform23m.php?id="+del_id,
                cache:false,

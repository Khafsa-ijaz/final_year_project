...
if ($rowadv['pagename']=='Vouchers List')
{
   ...
   echo "<li class='active'><a class='menu-item' href='$rowadv[link]' data-i18n='nav.vertical_nav.vertical_nav_fixed'>$rowadv[pagename]</a>
	...
	<div class="row">
      ...
      ?>
                      <h4 class="card-title">Welcome you are Viewing Fee Periods</h4>
            ...
            <div   class="card">
                ...
                <div class="card-content collapse show">
                 ...
                 <div class="s text-center " role="" aria-label="Basic example"> 
                      <form method="POST" action="feeperiod">
                            ...
                            <thead>
                                <tr>
                                   <th style="background-color:#304A7B;color: white ">S.#</th>
                                      <th style="background-color:#304A7B;color: white ">Month</th>
                                    <th style="background-color:#304A7B;color: white  ">Due Date</th>
                                          <th style="background-color:#304A7B;color: white  ">Fine</th>
  ...
  $count=0;
            ...
            $sql = "SELECT * FROM feeperiod order by id DESC";
            $result = mysqli_query($conn, $sql);

            if (mysqli_num_rows($result) > 0) {
            // output data of each row
             while($row = mysqli_fetch_assoc($result))  {
            $a1=date("F",strtotime($row['duedate']));
      $a2=date("Y",strtotime($row['duedate']));
      $a3=date("d",strtotime($row['duedate']));
      $format=$a3."-".$a1."-".$a2;


         $a1=date("F",strtotime($row['month']));
      $a2=date("Y",strtotime($row['month']));
      ...
      $format2=$a1."-".$a2;
           echo " <tr>
              <td>".++$count."</td>
                  ...
                  <td><b>".number_format($row['fine'])."</b></td>
                     <td>
                      <div class='btn-group' role='group' aria-label='Basic example'>
                                           <a href=id.php?id=".$row['id']."><button type='button' class='btn btn-icon btn-outline-warning btn-sm'><i class='fa fa-eye'></i></button></a>
...
$(".cc").click(function(e) {
 ...
 var del_id= $(this).attr('id');
 swal({
  title: "Are You Sure to Delete Fee Period?",
  icon: "warning",
  buttons: true,
  dangerMode: true,
})
 .then((willDelete) => {
  if (willDelete) {
     $.ajax({
                url:"deleteperiod.php?id="+del_id,
                cache:false,
                success:function(result){

...
$sql11 = "SELECT name FROM employees where id='$id'";
...
$a12=date("F",strtotime($date));
   ...
   $this->SetTextColor(0,0,0);
    ...
    $this->Cell(160,10,' DATEWISE ' .$status. ' TASK REPORT',0,0,'C');

 $this->Ln(7);
  $this->Cell(280,10,'DATE: ' .$format2.'***'.$format3,0,0,'C');
  
   $this->Ln(7);
  $this->Cell(280,10,'EMPLOYEE: ' .$name,0,0,'C');
    $this->Ln(7);
    }
    else{
...
if ($status=='ALL')
{
  $sql = "SELECT * FROM tasks where sdate between '$date' and '$date1'  and empid='$id' order by status";  
}
else
{
$sql = "SELECT * FROM tasks where sdate between '$date' and '$date1' and status='$status' and empid='$id' order by status";
}

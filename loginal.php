...
<section class="w3l-bootstrap-header">
  ...
  <div class="breadcrum-bg py-sm-5 py-4">
    <div class="container py-lg-3">
      <h2>About Us</h2>
      <p><a href="index12.php">Home</a> &nbsp; / &nbsp; Login</p>
    </div>
  </div>
</section>
<section class="w3l-content-with-photo-4">
    <div id="content-with-photo4-block" class="d-flex justify-content-center align-items-center vh-100">
        <div class="container text-center" >
            <div class="logo mb-4">
                <img src="logo.png" alt="University Logo" style="width: 10%;">
            </div>
            <h6 class="mb-4" style="color:#5B8343">Welcome to Women University Alumni Portal!</h6>
            <?php
            if (isset($_POST['log_in_client'])) {
                $u_email = $_POST['u_email'];
                $u_pass = $_POST['u_pass'];
                if (filter_var($u_email, FILTER_VALIDATE_EMAIL) && strpos($u_email, '.com') !== false) {
                    $checking_qry = $conn->query("SELECT * FROM accounts WHERE email = '$u_email' AND pass='$u_pass'");
                    if (mysqli_num_rows($checking_qry) > 0) {
                        $_SESSION['std_email'] = $u_email;
                        echo "<script>window.location='login.php?auth_succ=" . urlencode("Login successfully .") . "'</script>";
                    } else {
                        echo "<script>window.location='login.php?auth_err=" . urlencode("Invalid credential .") . "'</script>";
                    }
                } else {
                    echo "<script>window.location='login.php?auth_err=" . urlencode("Invalid Email .") . "'</script>";
                }
            }
            ?>
            <form action="" method="post" class="mx-auto" style="max-width: 400px; ">
                <div class="form-group mb-3">
                    ...
                    <input type="email" id="email" name="u_email" class="form-control" placeholder="Enter your email" required>
                </div>
                <div class="form-group mb-3">
                    <label for="password">Password:</label>
                    <input type="password" id="password" name="u_pass" class="form-control" placeholder="Password" required>
                    <a href="forgetpassword.php" class="forgot-password d-block mt-2">Forgot Password?</a>
                </div>
                <button type="submit" name="log_in_client" class="btn btn-primary-outlined w-100" style="background-color:#5B8343;color: white;">Sign in</button>
                <p class="mt-3" style="color:#5B8343">New to Women University? <a href="createAccount.php">Create new account</a></p>
            </form>
        </div>
    </div>
</section>

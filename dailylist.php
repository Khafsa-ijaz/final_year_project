...
if ($rowadv['pagename']=='Daily Report')
{
...
else
{
   echo "<li><a class='menu-item' href='$rowadv[link]' data-i18n='nav.vertical_nav.vertical_nav_fixed'>$rowadv[pagename]</a>
    ...
    <?php
      ...
      $format=$a3."-".$a1."-".$a2;
      ?>
                      <h4 class="card-title">Staff Attendance List  <b><i style="color:red">Date: </i><u><?php echo $format?></u> <i style="color:green"></i></u></b></h4>
  ...
  $count=0;
   ...
   $sqli = "SELECT emp_id,status FROM tattendence  where date='$date' order by emp_id ASC ";
...
dom: 'Blfrtip',
...

        {
           extend: 'csv',
           footer: false,
                      messageTop: 'Staff Attendace List  Date:<?php echo $format?>'

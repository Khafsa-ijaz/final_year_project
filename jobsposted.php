...
<section class="w3l-about-breadcrum">
  <div class="breadcrum-bg py-sm-5 py-4">
    <div class="container py-lg-3">
      <h2>Jobs Posted</h2>
      <p><a href="index12.php">Home</a> &nbsp; / &nbsp;<a href="job.php"> Jobs</a>&nbsp; / &nbsp;Jobs View</p>
    </div>
  </div>
</section>
...
<section class="w3l-content-with-photo-4">
        ...
        <div class="container py-md-5">
            <div class="cwp4-two row">
                ...
                <div class="cwp4-text col-lg-8">
                     <?php
    $jobId = $_GET['job_id'];
    $jb_qry = $conn->query("SELECT * FROM `jobs` WHERE id = '$jobId'");
    $jb_data = mysqli_fetch_array($jb_qry);
    ...
    if (isset($_POST['jobApply'])) {
        ...
        // Handle file upload
        if (isset($_FILES['applicantResume']) && $_FILES['applicantResume']['error'] == 0) {
            $resume = $_FILES['applicantResume']['name'];
            $tmp_name = $_FILES['applicantResume']['tmp_name'];
            $upload_dir = 'pictures/';  // Directory to save uploaded files
            move_uploaded_file($tmp_name, $upload_dir . $resume);
        } else {
        ...
        $stmt = $conn->prepare("INSERT INTO job_applications (name, email, phone, resume, message) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssss", $name, $email, $phone, $resume, $message);
    ...
    ?>
      <div class="container-fluid py-4 px-2 px-md-5 my-2 my-md-5">
        <div class="job-title1" style="color:#1090CD; font-weight: bold;"><?php echo htmlspecialchars($jb_data['title']); ?></div>
        <div class="posted-by1" style="color:#5B8343">Posted by <?php echo htmlspecialchars($jb_data['post_by']); ?> <span style="color: #1090CD;"> <?php echo htmlspecialchars($jb_data['c_date']); ?> </span></div>

        <div class="job-detail1s1" >
            <div class="job-detail1" ><strong style="color:#5B8343">Employer Name:</strong> <?php echo htmlspecialchars($jb_data['name']); ?></div>
            <div class="job-detail1"><strong style="color:#5B8343">Job Type:</strong> <?php echo htmlspecialchars($jb_data['job_type']); ?></div>
            <div class="job-detail1"><strong style="color:#5B8343">Last Date to Apply:</strong> <?php echo htmlspecialchars($jb_data['l_d_a']); ?></div>
        </div>

        <div class="job-description1">
            <p><?php echo htmlspecialchars($jb_data['parag']); ?></p>
            <p><?php echo htmlspecialchars($jb_data['dscr']); ?></p>
        </div>
        <button class="btn btn-success-outline" data-toggle="modal" data-target="#jobApplyModal" style="background-color:#5B8343; color: white;font-weight: bold;">Apply</button>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="jobApplyModalLabel">Job Application</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="jobApplyForm" action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" class="form-control" id="applicantEmail" name="applicantEmail" placeholder="Enter your email" required>
                        </div>
                        <div class="form-group">
                            <label for="applicantPhone">Phone Number</label>
                            <input type="tel" class="form-control" id="applicantPhone" name="applicantPhone" placeholder="Enter your phone number" required>
                        </div>
                        <div class="form-group">
                            <label for="applicantResume">Resume</label>
                            <input type="file" class="form-control-file" id="applicantResume" name="applicantResume" required>
                        </div>
                        <div class="form-group">
                            <label for="applicantMessage">Message</label>
                            <textarea class="form-control" id="applicantMessage" name="applicantMessage" rows="3" placeholder="Why are you a good fit for this job?" required></textarea>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary-outline" data-dismiss="modal" style="background-color:#1090CD;color: white;">Close</button>
                            <button type="submit" class="btn btn-primary-outline" name="jobApply" style="background-color:#5B8343;color: white;">Submit Application</button>
                        </div>
                    </form>
                </div>
            </div>
          ...
          <div class="d-grid grid-col-4 footer-top-29">
              <div class="footer-list-29 footer-1">
                  ...
                  <ul>
                      ...
                      <li><a href="goharrehman@wuajk.edu.pk" class="mail"><span class="fa fa-envelope-open-o"></span> goharrehman@wuajk.edu.pk</a></li>
                  </ul>
